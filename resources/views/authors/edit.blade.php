      ...
      <div class="grid grid-cols-1 gap-6">
        ...
        <div>
          ...
          <textarea name="biography" id="biography" rows="4"
            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('biography', $author->biography) }}</textarea>
          @error('biography')
            <span class="text-red-600 text-sm">{{ $message }}</span>
          @enderror
        </div>
