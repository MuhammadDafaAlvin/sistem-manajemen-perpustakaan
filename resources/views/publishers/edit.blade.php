...
@section('content')
  <div class="bg-[#1a1a1a] shadow sm:rounded-3xl mt-4">
    <div class="px-8 py-10 rounded-t-3xl">
      <h3 class="text-3xl font-bold text-white tracking-tight">Edit Penerbit</h3>
      <p class="mt-2 text-base text-gray-400">Perbarui informasi penerbit dengan data yang sesuai.</p>
    </div>
      ...
      <div class="grid grid-cols-1 gap-6">
        ...
        <div>
          ...
          <textarea name="address" id="address" rows="4"
            class="mt-1 block w-full bg-[#2a2a2a] text-white border-gray-600 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm">{{ old('address', $publisher->address) }}</textarea>
          @error('address')
            <span class="text-red-400 text-sm">{{ $message }}</span>
          @enderror
        </div>
